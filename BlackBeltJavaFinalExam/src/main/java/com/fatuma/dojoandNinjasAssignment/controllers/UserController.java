	...
	// BindingResult that allows for automatically writing the form data into a new object that we bind to the JSP page using the view model, that is, the Model model that you're accustomed to using.
	 ...
	 public String homeRoute(Model viewModel,HttpSession session ) {
		 ...
		 viewModel.addAttribute("currentUser", this.uService.findById(userId));
	        ...
	        // Creating a list of NameModel objects to hold the names
	        List<GameModel> Listnames = new ArrayList<>();
	        for (GameModel name : Listnames) {
	            GameModel nameModel = new GameModel();
//	            nameModel.setName(name);
	            Listnames.add(nameModel);
	        }

	        // Sorting the names alphabetically based on the 'name' property of NameModel
	        Collections.sort(Listnames, Comparator.comparing(GameModel::getName));

	        // Adding the sorted list to the model for rendering in the view
	        viewModel.addAttribute("allGames", Listnames);
		 viewModel.addAttribute("allGames", this.gService.getAll());
//		 this.gService.getAllGameMyOrder(newGame.getName());
